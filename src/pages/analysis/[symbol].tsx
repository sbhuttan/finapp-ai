...
const DeeperAnalysisPage: NextPage<Props> = ({ symbol }) => {
  ...
  const loadAnalyses = async () => {
    try {
      ...
      // Load all analyses concurrently
      const [marketRes, sentimentRes, riskRes] = await Promise.all([
        axios.get(`/api/analysis/market/${symbol}`),
        axios.get(`/api/analysis/sentiment/${symbol}`),
        axios.get(`/api/analysis/risk/${symbol}`)
      ])
